...
class KDNode(Node):
    ...
    def create_subnode(self, data):
        ...
        return self.__class__(data,            # 相当于KDNode
    ...
    def search_knn(self, point, k, dist=None):
        ...
        if dist is None:
            get_dist = lambda n: n.dist(point) # 计算两点之间的距离
        else:
            get_dist = lambda n: dist(n.data, point)

        results = []

        self._search_node(point, k, results, get_dist, itertools.count()) # iterator 生成一个从0开始的无限的迭代器
    ...
    def _search_node(self, point, k, results, get_dist, counter):
        print('in search',self.data)
        if not self:
            return

        nodeDist = get_dist(self)
        # print(self.data, nodeDist)
        ...
        if len(results) >= k:
            if -nodeDist > results[0][0]:
                heapq.heapreplace(results, item)  #replace secondary min value with most min value
        else:
        ...
        if point[self.axis] < split_plane:
            ...
            print('right',self.right is None)
    ...
    def search_nn(self, point, dist=None):
        ...
        return next(iter(self.search_knn(point, 1, dist)), None)  # iter() 生成一个迭代器，并去下一个作为返回值
...
def create(point_list=None, dimensions=None, axis=0, sel_axis=None): # 递归创建一个kdtree
    ...
    sel_axis = sel_axis or (lambda prev_axis: (prev_axis+1) % dimensions) # 定义孩子节点为维度的规则，好的写法
    ...
    point_list.sort(key=lambda point: point[axis]) #按照指定维度进行排序,并进行的是深拷贝
